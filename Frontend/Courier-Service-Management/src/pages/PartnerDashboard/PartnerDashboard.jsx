...
export default function PageDashboard() {
  ...
  useEffect(() => {
    const fetchPartnerData = async () => {
      try {
        ...
        if (profileResponse.data.responseStatus === "SUCCESS") {
  ...
  const handleToggleOnline = async () => {
    ...
    setPartnerProfile((prev) => prev ? { ...prev, isOnline: next } : prev);
    ...
    try {
      ...
      if (resp?.data?.status === "SUCCESS") {
        setPartnerProfile((prev) => prev ? { ...prev, isOnline: !!resp.data.isOnline } : prev);
      } else {
        // rollback
        setPartnerProfile((prev) => prev ? { ...prev, isOnline: !next } : prev);
      }
  ...
  const user = partnerProfile 
    ? { name: welcomeName, email: partnerProfile.email }
    : { name: "Partner" };
          ...
          {activeTab === "available" && (
            isOnline ? (
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <h3 className="font-semibold text-gray-800 mb-1">You’re offline</h3>
